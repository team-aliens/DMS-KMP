package team.aliens.dms.kmp.core.domain.usecase.notification

import team.aliens.dms.kmp.core.data.notification.repository.NotificationRepository
import team.aliens.dms.kmp.core.model.notification.NotificationTopicStatusModel

class BatchUpdateNotificationTopicUseCase(
    private val notificationRepository: NotificationRepository,
) {
    suspend operator fun invoke(topics: List<NotificationTopicStatusModel.TopicGroup.TopicSubscription>) =
        notificationRepository.batchUpdateNotificationTopic(topics = topics)
}
