...
internal class KtorAuthDataSource(private val client: HttpClient) : NetworkAuthDataSource {
    override suspend fun signIn(request: SignInRequest): Result<TokenDto> = runCatching {
    ...
    override suspend fun reissueToken(request: ReissueRequest): Result<TokenDto> =
        kotlin.runCatching {
            client.put("/auth/reissue") {
                header("refresh-token", request.header.refreshToken)
            }.body()
        }
}
